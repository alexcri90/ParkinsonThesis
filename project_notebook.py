...
# Cell 49: Enhanced GPU-Optimized DataLoader Implementation
import torch
from torch.utils.data import Dataset, DataLoader
import numpy as np
import pydicom
...
class EnhancedDaTScanDataset(Dataset):
    """
    Enhanced DaTScan dataset with optimized memory management.
    Implements controlled prefetching and memory-aware loading.
    """
    def __init__(self, dataframe, transform=None, prefetch_factor=2, max_workers=2):
        self.df = dataframe
        self.transform = transform
        self.prefetch_factor = min(prefetch_factor, 2)  # Limit prefetch factor
        self.max_workers = max_workers
        self.prefetch_queue = queue.Queue(maxsize=self.prefetch_factor)
        self._stop_event = threading.Event()

        # Track memory usage
        self.mem_threshold = 0.85  # Stop prefetching if memory usage exceeds 85%

        # Start prefetching thread
        self._start_prefetching()

    def _start_prefetching(self):
        """Start controlled asynchronous data prefetching."""
        def prefetch_worker():
            idx = 0
            with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                while not self._stop_event.is_set():
                    ...
                    if self._check_memory_usage():
                        # Sleep if memory usage is too high
                        time.sleep(1)
                        continue

                    # Submit only a few items at a time
                    futures = []
                    for _ in range(self.max_workers):
                        if idx >= len(self.df):
                            idx = 0  # Reset to beginning
    ...
    def _check_memory_usage(self):
        ...
        return memory_info.percent > (self.mem_threshold * 100)

    def _load_item(self, idx):
        """Load and preprocess a single item with memory management."""
        try:
            file_path = self.df.iloc[idx]["file_path"]
            ...
            if volume.ndim != 3:
                print(f"Skipping file {file_path}: Expected 3D volume, got shape {volume.shape}")
                return None

            # Preprocess with memory cleanup
            try:
                volume = self._preprocess_volume(volume)
            except MemoryError:
                print(f"Memory error preprocessing {file_path}")
                return None
            finally:
                gc.collect()
    ...
    def _preprocess_volume(self, volume):
        ...
        try:
            ...
            resized = np.zeros((128, 128, 128), dtype=np.float32)
            current_shape = volume.shape

            # Process one dimension at a time
            for i in range(3):
                if current_shape[i] > 128:
                    # Center crop
                    start = (current_shape[i] - 128) // 2
                    if i == 0:
                        volume = volume[start:start+128, :, :]
                    elif i == 1:
                        volume = volume[:, start:start+128, :]
                    else:
                        volume = volume[:, :, start:start+128]
                elif current_shape[i] < 128:
                    # Calculate padding
                    pad_total = 128 - current_shape[i]
                    pad_before = pad_total // 2
                    pad_width = [(0, 0), (0, 0), (0, 0)]
                    pad_width[i] = (pad_before, pad_total - pad_before)
                    volume = np.pad(volume, pad_width=pad_width, mode='constant', constant_values=0)

                # Update current shape after modification
                current_shape = volume.shape

                # Force garbage collection after each dimension
                gc.collect()
    ...
    def __getitem__(self, idx):
        """Get item with memory-aware fallback."""
        try:
            # Try to get from prefetch queue with timeout
            item = self.prefetch_queue.get(timeout=2)
            return item
        except queue.Empty:
            # Fallback to direct loading if queue is empty
            print("Prefetch queue empty, loading directly...")
            return self._load_item(idx)
        except Exception as e:
            print(f"Error in __getitem__: {e}")
            # Return emergency fallback item
            return self._load_item(idx)

    def __del__(self):
        """Cleanup prefetching thread and memory."""
        self._stop_event.set()
        gc.collect()

def create_enhanced_dataloaders(df, batch_size=4, train_split=0.8):
    """Create dataloaders with memory-optimized settings."""
    # Filter out rows with invalid files
    valid_files = []
    for idx, row in tqdm(df.iterrows(), desc="Validating files", total=len(df)):
        try:
            ds = pydicom.dcmread(row["file_path"])
            if ds.pixel_array.ndim == 3:
                valid_files.append(row)
        except Exception as e:
            print(f"Error checking file {row['file_path']}: {e}")
        finally:
            gc.collect()

    valid_df = pd.DataFrame(valid_files)
    print(f"Found {len(valid_df)} valid 3D volumes out of {len(df)} total files")
    ...
    # Create datasets with controlled prefetching
    train_dataset = EnhancedDaTScanDataset(
        train_df,
        prefetch_factor=2,
        max_workers=2
    )
    val_dataset = EnhancedDaTScanDataset(
        val_df,
        prefetch_factor=2,
        max_workers=2
    )
    ...
    train_loader = DataLoader(
        ...
        batch_size=batch_size,
        shuffle=True,
        num_workers=0,  # No multiprocessing to better control memory
        pin_memory=True,
...
# Cell 50: VAE-GAN Model Architecture
import torch
import torch.nn as nn
import torch.nn.functional as F
from collections import OrderedDict
...
class ConvBlock(nn.Module):
    """3D Convolutional block with batch normalization and leaky ReLU."""
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1):
        super().__init__()
        self.block = nn.Sequential(OrderedDict([
            ('conv', nn.Conv3d(in_channels, out_channels, kernel_size, stride, padding)),
...
class Encoder(nn.Module):
    """VAE Encoder network for 128³ brain volumes."""
    def __init__(self, latent_dim=256):
        super().__init__()
...
class Decoder(nn.Module):
    """VAE Decoder network with skip connections."""
    def __init__(self, latent_dim=256):
        super().__init__()
...
class Discriminator(nn.Module):
    """3D Discriminator network for 128³ volumes."""
    def __init__(self):
        super().__init__()

        self.model = nn.Sequential(
            # Initial feature extraction
            ConvBlock(1, 32, kernel_size=4, stride=2, padding=1),    # 128 -> 64

            # Intermediate layers
            ConvBlock(32, 64, kernel_size=4, stride=2, padding=1),   # 64 -> 32
            ConvBlock(64, 128, kernel_size=4, stride=2, padding=1),  # 32 -> 16
            ConvBlock(128, 256, kernel_size=4, stride=2, padding=1), # 16 -> 8
            ConvBlock(256, 512, kernel_size=4, stride=2, padding=1), # 8 -> 4
...
class VAEGAN(nn.Module):
    ...
    def forward(self, x):
        ...
        # Discriminate
        recon_pred = self.discriminator(recon)
        real_pred = self.discriminator(x)

        return recon, mu, log_var, recon_pred, real_pred

# Cell 51: Loss Functions and Training Configuration
class VAEGANLoss:
    """Combined loss function for VAE-GAN."""
    def __init__(self, reconstruction_weight=1.0, kl_weight=0.1, adversarial_weight=0.1):
    ...
    def compute_vae_loss(self, recon, target, mu, log_var, recon_pred):
        # Reconstruction loss
        recon_loss = self.recon_criterion(recon, target)
        ...
        adv_loss = self.adv_criterion(recon_pred, torch.ones_like(recon_pred))
    ...
    def compute_discriminator_loss(self, recon_pred, real_pred):
        # Discriminator loss for fake samples
        fake_loss = self.adv_criterion(recon_pred, torch.zeros_like(recon_pred))
...
class VAEGANConfig:
    """Configuration for VAE-GAN training."""
    def __init__(self):
        # Model parameters
        self.latent_dim = 256
        self.learning_rate_vae = 1e-4
        ...
        # Training parameters
        self.batch_size = 8  # Conservative for 12GB VRAM
        self.epochs = 100
        self.accumulation_steps = 4
...
# Cell 52: Training Loop Implementation
import torch.cuda.amp as amp
from pathlib import Path
import json
import time
...
class VAEGANCheckpointHandler:
    """Handles saving and loading of VAE-GAN checkpoints."""
    def __init__(self, checkpoint_dir, model_name):
        self.checkpoint_dir = Path(checkpoint_dir)
        self.model_name = model_name
        self.checkpoint_path = self.checkpoint_dir / f"{model_name}_checkpoint.pth"
    ...
    def save(self, model, vae_optimizer, disc_optimizer, schedulers, epoch, metrics):
        checkpoint = {
            'epoch': epoch,
            'model_state_dict': model.state_dict(),
            'vae_optimizer_state_dict': vae_optimizer.state_dict(),
            'disc_optimizer_state_dict': disc_optimizer.state_dict(),
            'vae_scheduler_state_dict': schedulers['vae'].state_dict() if schedulers['vae'] else None,
            'disc_scheduler_state_dict': schedulers['disc'].state_dict() if schedulers['disc'] else None,
            'metrics': metrics
        }
        torch.save(checkpoint, self.checkpoint_path)
    ...
    def load(self, model, vae_optimizer, disc_optimizer, schedulers):
        if not self.checkpoint_path.exists():
            return None
        ...
        if schedulers['vae'] and checkpoint['vae_scheduler_state_dict']:
            schedulers['vae'].load_state_dict(checkpoint['vae_scheduler_state_dict'])
        if schedulers['disc'] and checkpoint['disc_scheduler_state_dict']:
            schedulers['disc'].load_state_dict(checkpoint['disc_scheduler_state_dict'])

        return checkpoint['epoch'], checkpoint['metrics']

def train_vaegan(model, train_loader, val_loader, config=None):
    """Training loop optimized for RTX 4070 Ti with mixed precision training."""
    if config is None:
        config = VAEGANConfig()
    ...
    schedulers = {
        ...
        'disc': torch.optim.lr_scheduler.ReduceLROnPlateau(
            disc_optimizer, mode='min', factor=0.5, patience=5, verbose=True
        )
    }

    # Mixed precision training
    vae_scaler = amp.GradScaler(enabled=config.use_amp)
    disc_scaler = amp.GradScaler(enabled=config.use_amp)

    checkpoint_handler = VAEGANCheckpointHandler(config.checkpoint_dir, config.model_name)

    # Training tracking
    best_val_loss = float('inf')
    patience_counter = 0
    metrics = {
        'train_vae_losses': [], 'train_disc_losses': [],
        'val_vae_losses': [], 'val_disc_losses': [],
        'train_recon': [], 'val_recon': [],
        'train_kl': [], 'val_kl': [],
        'train_adv': [], 'val_adv': []
    }
    ...
    # Load checkpoint if available
    start_epoch = 0
    checkpoint_data = checkpoint_handler.load(model, vae_optimizer, disc_optimizer, schedulers)
    if checkpoint_data:
        start_epoch, metrics = checkpoint_data
        print(f"Resuming training from epoch {start_epoch + 1}")
    ...
    try:
        for epoch in range(start_epoch, config.epochs):
            # Training phase
            model.train()
            train_vae_loss = train_disc_loss = 0
            train_recon = train_kl = train_adv = 0
            ...
            for i, batch in enumerate(train_pbar):
                ...
                # Train Discriminator
                with amp.autocast(enabled=config.use_amp):
                    recon, mu, log_var, recon_pred, real_pred = model(volumes)
                    disc_loss = criterion.compute_discriminator_loss(recon_pred.detach(), real_pred)
                    disc_loss = disc_loss / config.accumulation_steps
                ...
                # Train VAE
                with amp.autocast(enabled=config.use_amp):
                    recon, mu, log_var, recon_pred, _ = model(volumes)
                    vae_loss, recon_loss, kl_loss, adv_loss = criterion.compute_vae_loss(
                        recon, volumes, mu, log_var, recon_pred
                    )
                    vae_loss = vae_loss / config.accumulation_steps

                vae_scaler.scale(vae_loss).backward()

                if (i + 1) % config.accumulation_steps == 0:
                    vae_scaler.step(vae_optimizer)
                    vae_scaler.update()
                    vae_optimizer.zero_grad()

                # Track metrics
                train_vae_loss += vae_loss.item() * config.accumulation_steps
                train_disc_loss += disc_loss.item() * config.accumulation_steps
                train_recon += recon_loss.item()
                train_kl += kl_loss.item()
                train_adv += adv_loss.item()

                train_pbar.set_postfix({
                    'vae_loss': vae_loss.item() * config.accumulation_steps,
                    'disc_loss': disc_loss.item() * config.accumulation_steps
                })

                # Clean up
                del volumes, recon, mu, log_var, recon_pred, real_pred
                torch.cuda.empty_cache()

            # Validation phase
            model.eval()
            val_vae_loss = val_disc_loss = 0
            val_recon = val_kl = val_adv = 0
            ...
            with torch.no_grad():
                for batch in val_pbar:
                    ...
                    disc_loss = criterion.compute_discriminator_loss(recon_pred, real_pred)
            ...
            # Update metrics
            metrics['train_vae_losses'].append(train_vae_loss / len(train_loader))
            metrics['train_disc_losses'].append(train_disc_loss / len(train_loader))
            metrics['val_vae_losses'].append(val_vae_loss / len(val_loader))
            metrics['val_disc_losses'].append(val_disc_loss / len(val_loader))
            metrics['train_recon'].append(train_recon / len(train_loader))
            metrics['val_recon'].append(val_recon / len(val_loader))
            metrics['train_kl'].append(train_kl / len(train_loader))
            metrics['val_kl'].append(val_kl / len(val_loader))
            metrics['train_adv'].append(train_adv / len(train_loader))
            metrics['val_adv'].append(val_adv / len(val_loader))

            # Update schedulers
            schedulers['vae'].step(val_vae_loss / len(val_loader))
            schedulers['disc'].step(val_disc_loss / len(val_loader))

            # Save checkpoint
            checkpoint_handler.save(
                model, vae_optimizer, disc_optimizer, schedulers,
                epoch, metrics
            )

            # Early stopping check
            val_total_loss = val_vae_loss / len(val_loader)
            ...
            # Print epoch summary
            print(f"\nEpoch {epoch+1}/{config.epochs}")
            print(f"Train VAE Loss: {train_vae_loss/len(train_loader):.6f}")
            print(f"Train Disc Loss: {train_disc_loss/len(train_loader):.6f}")
            print(f"Val VAE Loss: {val_vae_loss/len(val_loader):.6f}")
            print(f"Val Disc Loss: {val_disc_loss/len(val_loader):.6f}")
...
# Cell 53: Evaluation and Visualization Functions
import matplotlib.pyplot as plt
import seaborn as sns
from skimage.metrics import structural_similarity as ssim
import numpy as np

def load_trained_vaegan(checkpoint_dir, model_name='vaegan'):
    """Load trained VAE-GAN model and metadata."""
    checkpoint_path = Path(checkpoint_dir) / f"{model_name}_checkpoint.pth"
    metadata_path = Path(checkpoint_dir) / f"{model_name}_metadata.json"
...
def plot_training_history(metrics):
    ...
    # Plot VAE and Discriminator losses
    plt.subplot(2, 3, 1)
    plt.plot(metrics['train_vae_losses'], label='Train VAE')
    plt.plot(metrics['val_vae_losses'], label='Val VAE')
    plt.plot(metrics['train_disc_losses'], label='Train Disc')
    plt.plot(metrics['val_disc_losses'], label='Val Disc')
    ...
    # Plot reconstruction loss
    plt.subplot(2, 3, 2)
    plt.plot(metrics['train_recon'], label='Train')
    plt.plot(metrics['val_recon'], label='Val')
    plt.title('Reconstruction Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    ...
    # Plot KL divergence
    plt.subplot(2, 3, 3)
    plt.plot(metrics['train_kl'], label='Train')
    plt.plot(metrics['val_kl'], label='Val')
    plt.title('KL Divergence')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    ...
    plt.subplot(2, 3, 4)
    plt.plot(metrics['train_adv'], label='Train')
    plt.plot(metrics['val_adv'], label='Val')
...
def evaluate_reconstruction_quality(model, val_loader, num_samples=5):
    ...
    with torch.no_grad():
        for batch in val_loader:
            if len(metrics_per_sample) >= num_samples:
                break

            volumes = batch['volume'].to(device)
            recon, _, _, _, _ = model(volumes)

            for idx in range(volumes.shape[0]):
                if len(metrics_per_sample) >= num_samples:
                    break

                orig = volumes[idx, 0].cpu().numpy()
                rec = recon[idx, 0].cpu().numpy()

                # Compute metrics
                mse = np.mean((orig - rec) ** 2)
                ssim_score = ssim(orig, rec, data_range=orig.max() - orig.min())
                ...
                plt.title(f'Reconstructed - Axial\nMSE: {mse:.4f}')
                plt.axis('off')

                plt.subplot(2, 3, 5)
                plt.imshow(rec[:, rec.shape[1]//2], cmap='gray')
                plt.title(f'Reconstructed - Sagittal\nSSIM: {ssim_score:.4f}')
                plt.axis('off')
                ...
                plt.suptitle(f'Sample {len(metrics_per_sample)}')
                plt.tight_layout()
                plt.show()

    return metrics_per_sample

def generate_synthetic_samples(model, num_samples=5):
    ...
    with torch.no_grad():
        ...
        # Generate samples
        synthetic_samples = model.decoder(z, dummy_skip_connections)
        ...
        for i in range(num_samples):
            ...
            # Plot axial view
            plt.subplot(num_samples, 3, i*3 + 1)
            plt.imshow(sample[sample.shape[0]//2], cmap='gray')
            plt.title('Synthetic - Axial' if i == 0 else '')
            plt.axis('off')

            # Plot sagittal view
            plt.subplot(num_samples, 3, i*3 + 2)
            plt.imshow(sample[:, sample.shape[1]//2], cmap='gray')
            plt.title('Synthetic - Sagittal' if i == 0 else '')
            plt.axis('off')

            # Plot coronal view
            plt.subplot(num_samples, 3, i*3 + 3)
            plt.imshow(sample[:, :, sample.shape[2]//2], cmap='gray')
            plt.title('Synthetic - Coronal' if i == 0 else '')
            plt.axis('off')
...
# Cell 54: Training Implementation
if __name__ == "__main__":
    try:
        print("Initializing VAE-GAN training...")

        # Initialize model and configuration
        config = VAEGANConfig()
        model = VAEGAN(latent_dim=config.latent_dim)
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        model = model.to(device)

        # Create enhanced dataloaders
        print("\nCreating enhanced dataloaders...")
        train_loader, val_loader = create_enhanced_dataloaders(df, batch_size=config.batch_size)

        print("\nStarting training process...")
        # Start training
        metrics = train_vaegan(model, train_loader, val_loader, config)

        print("\nTraining completed successfully!")

    except KeyboardInterrupt:
        print("\nTraining interrupted by user. Progress has been saved to checkpoint.")
        print("Run the evaluation cell to analyze results from the last checkpoint.")
    except Exception as e:
