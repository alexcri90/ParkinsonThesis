...
# Cell 21.5: Analysis of Latent Space Distributions and Alpha Values
...
# Cell 25: Identifying Patients with Multiple Exams
import os
import re
import pandas as pd
import numpy as np
...
# Cell 27: Visualizing Patient Identity in Latent Space
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
...
def identify_longitudinal_patients(dataloader, min_exams=3, max_patients=10):
    ...
    # Time span distribution for longitudinal patients
    plt.figure(figsize=(10, 6))  # Fixed the syntax error here
    sns.histplot(data=longitudinal_df, x='time_span_days', hue='group', bins=20, multiple='stack')
    plt.title('Time Span of Exams for Longitudinal Patients')
    plt.xlabel('Time Span (days)')
    plt.ylabel('Number of Patients')
    ...
    # Modified selection strategy - prioritize patients with MOST exams
    # while ensuring representation from all groups
    selected_patients = []

    # Calculate max exams per group
    max_exams_per_group = {
        group: longitudinal_df[longitudinal_df['group'] == group]['num_exams'].max()
        for group in longitudinal_df['group'].unique()
    }

    # Set minimum threshold for each group (at least 3 exams or the group maximum)
    min_exams_threshold = {
        group: max(min_exams, max_exams_per_group[group] - 1)  # Allow at most 1 fewer than max
        for group in max_exams_per_group
    }

    print("\nSelection criteria:")
    for group, threshold in min_exams_threshold.items():
        print(f"  {group}: At least {threshold} exams (max in group: {max_exams_per_group[group]})")

    # Select patients meeting these criteria
    for group in longitudinal_df['group'].unique():
        # Select patients with exams at or above threshold
        group_patients = longitudinal_df[
            (longitudinal_df['group'] == group) &
            (longitudinal_df['num_exams'] >= min_exams_threshold[group])
        ]

        # If no patients meet threshold, take the top 2-3 patients
        if len(group_patients) == 0:
            group_patients = longitudinal_df[longitudinal_df['group'] == group].head(3)

        # Add to selection
        selected_patients.append(group_patients)

    # Combine and sort by number of exams
    selected_df = pd.concat(selected_patients).sort_values(by=['num_exams', 'time_span_days'], ascending=[False, False])

    # If we have too many, prioritize keeping patients with most exams
    if len(selected_df) > max_patients:
        print(f"More than {max_patients} patients meet the criteria, selecting top {max_patients} by exam count")
        # Make sure we have at least 1 from each group
        group_min_df = pd.DataFrame()
        for group in selected_df['group'].unique():
            group_min_df = pd.concat([
                group_min_df,
                selected_df[selected_df['group'] == group].head(1)
            ])

        # Fill remaining slots with patients with most exams
        remaining = max_patients - len(group_min_df)
        selected_df = pd.concat([
            group_min_df,
            selected_df[~selected_df['patient_id'].isin(group_min_df['patient_id'])].head(remaining)
        ])

    # Sort final selection by exam count
    selected_df = selected_df.sort_values(by='num_exams', ascending=False)
...
# Run the function to identify longitudinal patients
# Set min_exams=3 to ensure sufficient longitudinal data
patient_data, selected_patients = identify_longitudinal_patients(val_loader, min_exams=3, max_patients=10)